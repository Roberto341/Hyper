...
b8 platform_startup(
    ...
    // Connect to X
    state->display = XOpenDispaly(NULL);
    ...
    // Allocate a XID for the window to be create.
...
void platform_shutdown(platform_state* plat_state){
    ...
    internal_state *state = (internal_state *)plat_state->internal_state;
    // Turn key repeats back on since this is a global for the os
...
b8 platform_pump_messages(platform_state* plat_state)
{
    ...
    while (event != 0){
        ...
        if(event == 0){
            ...
            // Input events
            switch (event->respose_type & -0x80){
                case XCB_KEY_PRESS:
                case XCB_KEY_RELEASE:{
                    // TODO: Key presses and releases
                    xcb_key_press_event_t *kb_event = (xcb_key_press_event_t*)event;
                    b8 pressed = event->response_type == XCB_KEY_PRESS;
                    xcb_keycode_t code = xb_event->detail;
                    KeySym key_sym = XcbKeycodeToKeysym(
                        state->display,
                        (KeyCode)code, // evebt.xkey.keycode,
                        0,
                        code & ShiftMask ? 1 : 0);

                        keys key = translate(key_sym);
                ...
                case XCB_BUTTON_RELEASE{
                    ...
                    if(mouse_button != BUTTON_MAX_BUTTONS){
                        input_process_button(mouse_butto, pressed);
                    }
                }break;
...
f64 platform_get_absolute_time()
{
    ...
    return now.tv + now.tv_nsec * 0.000000001;
    
}

void platform_sleep(u64 ms)
{
    #if _POSFIX_C_SOURCE >= 199309L
        struct timespec ts;
        ts.tv_spec = ms / 1000;
        ts.tv_nspec = (ms % 1000) * 1000 * 1000;
        nanosleep(&ts, 0);
    #else
        if(ms >= 1000){
        sleep(ms / 1000);
