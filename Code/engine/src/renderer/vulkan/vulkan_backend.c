...
void regenerate_frambuffers(renderer_backend* backend, vulkan_swapchain* swapchain, vulkan_renderpass* renderpass);
...
b8 vulkan_renderer_backend_initialize(renderer_backend* backend, const char* application_name, struct platform_state* plat_state){
    ...
    app_info.pEngineName = "Hyper Engine";
    ...
    const char** require_extensions = darray_create(const char*);
    darray_push(require_extensions, &VK_KHR_SURFACE_EXTENSION_NAME);
    platform_get_required_extension_names(&require_extensions);

#if defined(_DEBUG)
    darray_push(require_extensions, &VK_EXT_DEBUG_UTILS_EXTENSION_NAME); // debug utils

    HDEBUG("Required extensions:");
    u32 length = darray_length(require_extensions);
    for(u32 i = 0; i < length; ++i){
        HDEBUG(require_extensions[i]);
    }
#endif
    create_info.enabledExtensionCount = darray_length(require_extensions);
    create_info.ppEnabledExtensionNames = require_extensions;
    ...
    // Obtain a list of avaliable validation layers
...
// NOTE: Debugger
    ...
    vulkan_renderpass_create(&context, &context.main_renderpass, 0.0f, 0.0f, context.framebuffer_width, context.framebuffer_height, 0.0f, 0.0f, 0.2f, 1.0f, 1.0f, 0);
   
   // Swapcain framebuffers.
    context.swapchain.framebuffers = darray_reserve(vulkan_framebuffer, context.swapchain.image_count);
    regenerate_frambuffers(backend, &context.swapchain, &context.main_renderpass);
...
b8 vulkan_renderer_backend_begin_frame(renderer_backend* backend, f32 delta_time){
    if(context.recreating_swapchain){
        ...
        if(!vulkan_result_is_success(result)){
            HERROR("vulkan_renderer_backend_begin_frame vKDeviceWaitIdel (1) failed: '%s'", vulkan_result_string(result, true));
            return false;
        }
    ...
    if(context.framebuffer_size_generation != context.framebuffer_size_last_generation){
        VkResult result = vkDeviceWaitIdle(context.device.logical_device);
        if(!vulkan_result_is_success(result)){
            HERROR("vulkan_renderer_backend_begin_frame vKDeviceWaitIdel (2) failed: '%s'", vulkan_result_string(result, true));
            return false;
        }
    ...
    if(!vulkan_fence_wait(&context, &context.in_flight_fences[context.current_frame], UINT64_MAX)){
        HWARN("In-flight fence wait failuer");
        return false;
    }
...
b8 create_buffers(vulkan_context* context)
{
    ...
    const u64 vertex_buffer_size = sizeof(vertex_3d) * 1024 * 1024; // 64 mb
    ...
    const u64 index_buffer_size = sizeof(u32) * 1024 * 1024; // 64 mb
    if(!vulkan_buffer_create(context, vertex_buffer_size, VK_BUFFER_USAGE_INDEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_TRANSFER_SRC_BIT, memory_property_flags, true, &context->object_index_buffer)){
