...
b8 vulkan_buffer_create(
    ...
    VkResult result = vkAllocateMemory(
        ...
        if(result != VK_SUCCESS){
            HERROR("Unable to create vulkan buffer because the memory allocation failed. Error: %i", result);
            return false;
        }
...
b8 vulkan_buffer_resize(
    ...
    if(result != VK_SUCCESS){
        HERROR("Unable to resize vulkan buffer the required memory allocation failed Error: %i", result);
        return false;
    }
    ...
    // Make sure anything potentially using these is finised.
...
void vulkan_buffer_load_data(vulkan_context* context, vulkan_buffer* buffer, u64 offset, u64 size, u32 flags, const char* data)
{
    void* data_ptr;
